...
impl Executor {
    ...
    pub async fn listen(&self) -> Result<()> {
        ...
        let l0_endpoint_abi = connections::get_abi_from_path("./abi/ArbitrumL0V2Endpoint.json")?;
        let _s_uln302 = connections::get_abi_from_path("./abi/ArbitrumSendLibUln302.json")?;
        let _r_uln302 = connections::get_abi_from_path("./abi/ArbitrumReceiveLibUln302.json")?;
        // Create a contract instance.
        let contract = contracts::create_contract_instance(&self.config, http_provider, l0_endpoint_abi)?;
                        ...
                        Ok(inner_log) => {
                            ...
                            while let Some(packet) = packet_sent_queue.pop_back() {
                                let call_builder = contract.function(
                                    "_executable",
                                    &[
                                        // TODO: maybe we need to extract header from packet...
